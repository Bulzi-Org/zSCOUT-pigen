...
# Always build for linux/arm64 — this image targets Raspberry Pi CM5 (aarch64) only
BASE_IMAGE=debian:bookworm
${DOCKER} build --platform linux/arm64 --build-arg BASE_IMAGE=${BASE_IMAGE} -t pi-gen "${DIR}"
...
# Check if qemu-aarch64-static and /proc/sys/fs/binfmt_misc are present (needed on non-arm64 hosts)
